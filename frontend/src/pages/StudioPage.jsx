...
const StudioHero = ({ studio }) => (
  <section className="relative h-[75vh] min-h-[520px] w-full overflow-hidden">
    ...
    <div className="relative h-full max-w-7xl mx-auto px-6 md:px-10 flex flex-col justify-end pb-20">
      ...
      <h1
        className="font-serif text-white text-6xl md:text-8xl lg:text-[120px] leading-none animate-fade-in-up"
        style={{ animationDelay: "0.1s", opacity: 0, animationFillMode: "forwards" }}
      >
        {studio.name}
      </h1>
      <div
        className="flex items-center gap-3 mt-6 text-[#e8e0d0] animate-fade-in-up"
        style={{ animationDelay: "0.25s", opacity: 0, animationFillMode: "forwards" }}
      >
        <MapPin size={16} />
        <span className="text-sm">{studio.address} — {studio.postal}</span>
      </div>
    </div>
  </section>
);

const StudioInfoBlock = ({ studio }) => (
...
const StudioServicesBlock = ({ studioName }) => (
  <section className="py-20 md:py-24 bg-[#f2ebdf] px-6 md:px-10">
    <div className="max-w-7xl mx-auto">
      <div className="reveal mb-12">
        <p className="text-xs tracking-[0.4em] uppercase text-[#8a7a5e] mb-4">Au studio de {studioName}</p>
      ...
      <div className="grid md:grid-cols-3 gap-px bg-[#d9cdb4]">
        {services.map((s, i) => (
          <div key={s.title} className="bg-[#f2ebdf] p-8 reveal" style={{ transitionDelay: `${i * 80}ms` }}>
